                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <select name="items[0][service_id]" 
                                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 service-select"
                                            required
                                            onchange="updateServicePrice(this, 0)">
                                ...
                                <div>
                                    ...
                                    <input type="number" 
                                           ...
                                           step="0.01" 
                                           min="0"
                                           class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 price-input"
                                           required
                                ...
                                <div>
                                    ...
                                    <input type="number" 
                                           name="items[0][parts_price]" 
                                           step="0.01" 
                                           min="0"
                                           value="0"
                                           class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 parts-price-input"
                                           onchange="calculateTotal()">
                                </div>
                            </div>
                        </div>
    ...
    <script>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="items[${itemIndex}][service_id]" 
                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 service-select"
                                required
                                onchange="updateServicePrice(this, ${itemIndex})">
                            <option value="">{{ __('invoice.select_service') }}</option>
                            @foreach($services as $service)
                                <option value="{{ $service->id }}" data-price="{{ $service->price }}">{{ $service->name }} - ${{ number_format($service->price, 2) }}</option>
                            @endforeach
        ...
        function updateServicePrice(select, index) {
            ...
            if (selectedOption.value) {
                ...
                priceInput.value = servicePrice.toFixed(2);
                calculateTotal();
            } else {
                priceInput.value = '';
                calculateTotal();
        ...
        function calculateTotal() {
            ...
            const partsPriceInputs = document.querySelectorAll('.parts-price-input');
            
            priceInputs.forEach((input, index) => {
                const price = parseFloat(input.value) || 0;
                const partsPrice = parseFloat(partsPriceInputs[index]?.value) || 0;
                total += price + partsPrice;
            });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Add event listeners to the first item's inputs
            const firstPriceInput = document.querySelector('input[name="items[0][price]"]');
            const firstPartsInput = document.querySelector('input[name="items[0][parts_price]"]');
            const firstServiceSelect = document.querySelector('select[name="items[0][service_id]"]');
            
            if (firstPriceInput) {
                firstPriceInput.addEventListener('change', calculateTotal);
                firstPriceInput.classList.add('price-input');
            }
            
            if (firstPartsInput) {
                firstPartsInput.addEventListener('change', calculateTotal);
                firstPartsInput.classList.add('parts-price-input');
            }
            
            if (firstServiceSelect) {
                firstServiceSelect.addEventListener('change', function() {
                    updateServicePrice(this, 0);
                });
                firstServiceSelect.classList.add('service-select');
            }
